...
# ===================== Config =====================
TARGETS = [
    ("c4", "Prob_4_Clearances"),
    ("c6", "Prob_6_Clearances"),
    ("c8", "Prob_8_Clearances"),
]
...
def resolve_models_dir() -> Path:
    candidates = [
        ROOT / "models" / "clearances",
        ROOT / "machine_learning" / "models" / "clearances",
    ]
    for p in candidates:
        if p.exists():
            return p.resolve()
    raise FileNotFoundError(
        "Could not find clearances model directory. Checked:\n" +
...
with engine.begin() as conn:
    for col in ["Prob_4_Clearances", "Prob_6_Clearances", "Prob_8_Clearances"]:
...
with engine.begin() as conn:
    for _, r in pred_out.iterrows():
        params = {
            "p4": to_python_scalar(r.get("Prob_4_Clearances")),
            "p6": to_python_scalar(r.get("Prob_6_Clearances")),
            "p8": to_python_scalar(r.get("Prob_8_Clearances")),
            "player": r["Player"],
        }

        sql = (
            'UPDATE player_precomputes '
            'SET "Prob_4_Clearances" = :p4, '
                '"Prob_6_Clearances" = :p6, '
                '"Prob_8_Clearances" = :p8 '
            'WHERE "Player" = :player'
        )
...
print(
    f"✅ Wrote clearances predictions for {len(pred_out)} players to player_precomputes "
