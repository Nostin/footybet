...
agg_dict = {
    **{k: "sum" for k in PLAYER_SUM_COLS},
...
tg["is_primary_home"] = tg.apply(
    lambda r: compute_is_primary_home(r.get("Team"), r.get("Venue")), axis=1
).astype(int)
tg["is_secondary_home"] = tg.apply(
    lambda r: is_secondary_home(r.get("Team"), r.get("Venue")), axis=1
).astype(int)
tg["is_home"] = tg.apply(
    lambda r: compute_is_home(r.get("Team"), r.get("Venue")), axis=1
).astype(int)
...
tg["is_interstate"] = tg.apply(
    lambda r: is_interstate(r.get("Team"), r.get("Venue")), axis=1
).astype(int)
...
def compute_features(g: pd.DataFrame) -> pd.DataFrame:
    ...
    for name, col in METRICS.items():
        ...
        # season-to-date features (season-scoped)
        season_avg = pd.Series(index=g.index, dtype=float)
        season_min = pd.Series(index=g.index, dtype=float)
        season_max = pd.Series(index=g.index, dtype=float)

        season_avg_home = pd.Series(index=g.index, dtype=float)
        season_min_home = pd.Series(index=g.index, dtype=float)
        season_max_home = pd.Series(index=g.index, dtype=float)

        season_avg_away = pd.Series(index=g.index, dtype=float)
        season_min_away = pd.Series(index=g.index, dtype=float)
        season_max_away = pd.Series(index=g.index, dtype=float)

        for _, idx in g.groupby("season").groups.items():
            idx = list(idx)

            s_season_raw = s_raw.loc[idx]
            x_season = _series_for_mode(s_season_raw)

            mask_home_season = mask_home.loc[idx]
            mask_away_season = mask_away.loc[idx]

            season_avg.loc[idx] = x_season.expanding(min_periods=1).mean()
            season_min.loc[idx] = x_season.expanding(min_periods=1).min()
            season_max.loc[idx] = x_season.expanding(min_periods=1).max()

            season_avg_home.loc[idx] = _season_to_date_subset(s_season_raw, mask_home_season, "mean")
            season_min_home.loc[idx] = _season_to_date_subset(s_season_raw, mask_home_season, "min")
            season_max_home.loc[idx] = _season_to_date_subset(s_season_raw, mask_home_season, "max")

            season_avg_away.loc[idx] = _season_to_date_subset(s_season_raw, mask_away_season, "mean")
            season_min_away.loc[idx] = _season_to_date_subset(s_season_raw, mask_away_season, "min")
            season_max_away.loc[idx] = _season_to_date_subset(s_season_raw, mask_away_season, "max")

        newcols[f"season_avg_{name}"] = season_avg
        newcols[f"season_min_{name}"] = season_min
        newcols[f"season_max_{name}"] = season_max

        newcols[f"season_avg_home_{name}"] = season_avg_home
        newcols[f"season_min_home_{name}"] = season_min_home
        newcols[f"season_max_home_{name}"] = season_max_home

        newcols[f"season_avg_away_{name}"] = season_avg_away
        newcols[f"season_min_away_{name}"] = season_min_away
        newcols[f"season_max_away_{name}"] = season_max_away

        # rolling features across all games regardless of season
        for w in WIN_SIZES:
            tmp = x.where(mask_roll_overall, np.nan)
...
tg_out = (
    tg.groupby(["Team"], group_keys=False)
...
print(
    f"✅ Wrote {len(tg_out):,} rows to '{dest}' and added opponent-derived metrics "
    f"(windows={WIN_SIZES}, stats_mode={args.stats_mode})."
)
print("🏟️ Added home-ground columns: is_primary_home, is_secondary_home, is_home, is_away.")
