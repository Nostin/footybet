...
# ===================== Config =====================
TARGETS = [("g1", "Prob_1_Goal"), ("g2", "Prob_2_Goals"), ("g3", "Prob_3_Goals")]
...
def resolve_models_dir() -> Path:
    candidates = [
        ROOT / "models" / "goals",
        ROOT / "machine_learning" / "models" / "goals",
    ]
    for p in candidates:
        if p.exists():
            return p.resolve()
    raise FileNotFoundError(
        "Could not find goals model directory. Checked:\n" +
...
def compute_train_means(expected_features):
    ...
    # apply same logical clipping used in training
    for col in [c for c in expected_features if c.startswith("accuracy_")]:
        if col in X.columns:
    ...
    # if a column is still all-NaN after cleaning, default it to 0
    means = means.replace([np.inf, -np.inf], np.nan).fillna(0.0)

    return means


# ===================== Exact training enrich_goals =====================
...
for key, out_col in TARGETS:
    ...
    # store as percentage 0-100 to match your disposals pattern
    pred_out[out_col] = np.clip(p_bl * 100.0, 0.0, 100.0)

# ===================== Persist =====================
with engine.begin() as conn:
    for col in ["Prob_1_Goal", "Prob_2_Goals", "Prob_3_Goals"]:
...
with engine.begin() as conn:
    for _, r in pred_out.iterrows():
        params = {
            "p1": to_python_scalar(r.get("Prob_1_Goal")),
            "p2": to_python_scalar(r.get("Prob_2_Goals")),
            "p3": to_python_scalar(r.get("Prob_3_Goals")),
            "player": r["Player"],
        }
...
print(
    f"✅ Wrote goal predictions for {len(pred_out)} players to player_precomputes "
