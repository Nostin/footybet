...
# ---------- Marks-only stat helpers ----------
METRIC = 'Marks'                # column name in player_stats
...
def summary_stats(s: pd.Series) -> pd.Series:
    ...
    # light cap to reduce outlier sway (adjust if you like)
    capped = s.clip(upper=15)
...
def marks_per100_tog(series_marks: pd.Series, series_tog: pd.Series) -> float:
    # Simple per-ToG% rate: marks * 100 / ToG%
    if series_marks is None or series_marks.empty or series_tog is None or series_tog.empty:
        return np.nan
    df = pd.DataFrame({'m': series_marks, 'tog': series_tog}).dropna()
    if df.empty:
        return np.nan
    df = df[df['tog'] > 0]  # avoid divide-by-zero
    if df.empty:
        return np.nan
    rates = df['m'] * 100.0 / df['tog']
...
# optional: add timeslot splits for parity:
# "Mark_Season_Day", "Mark_Season_Night", "Mark_Season_Twilight"
...
zero_cols = ["Mark_Season_ZeroRate"] + [f"Mark_{N}_ZeroRate" for N in WINDOWS]
rate_cols = ["Marks_per100ToG_Season"] + [f"Marks_per100ToG_{N}" for N in WINDOWS]
...
# ---------- ALTER TABLE: add marks columns if missing ----------
...
def compute_player_row(player: str) -> dict:
    ...
    if not p_season.empty:
        out.update(compute_for_split_metric(p_season, p_season[METRIC].notnull(), "Mark_Season"))
        out.update(compute_for_split_metric(p_season, p_season['Conditions'].astype(str).str.lower() == 'dry', "Mark_Season_Dry"))
        out.update(compute_for_split_metric(p_season, p_season['Conditions'].astype(str).str.lower() == 'wet', "Mark_Season_Wet"))
        hamask_season = p_season.apply(is_home_game_row, axis=1)
        out.update(compute_for_split_metric(p_season, hamask_season, "Mark_Season_Home"))
        out.update(compute_for_split_metric(p_season, ~hamask_season, "Mark_Season_Away"))

        # Zero rate + per-ToG rate (season)
        out["Mark_Season_ZeroRate"] = to_python_scalar(zero_rate(p_season[METRIC]))
        out["Marks_per100ToG_Season"] = to_python_scalar(
            marks_per100_tog(p_season[METRIC], p_season.get(TOG_COL, pd.Series(dtype=float)))
        )
    else:
        for pfx in season_prefixes:
            for sfx in stat_suffixes:
                out[f"{pfx}_{sfx}"] = np.nan
        out["Mark_Season_ZeroRate"] = np.nan
        out["Marks_per100ToG_Season"] = np.nan

    # Recent windows (all years)
    for N in WINDOWS:
        recent = p_all.sort_values('Date').tail(N)
        out.update(compute_for_split_metric(recent, recent[METRIC].notnull(), f"Mark_{N}"))

        last_dry = last_n_by(p_all, N, 'Conditions', 'dry')
        out.update(compute_for_split_metric(last_dry, last_dry[METRIC].notnull(), f"Mark_{N}_Dry"))

        last_wet = last_n_by(p_all, N, 'Conditions', 'wet')
        out.update(compute_for_split_metric(last_wet, last_wet[METRIC].notnull(), f"Mark_{N}_Wet"))

        last_home = last_n_home_away(p_all, N, home=True)
        out.update(compute_for_split_metric(last_home, last_home[METRIC].notnull(), f"Mark_{N}_Home"))

        last_away = last_n_home_away(p_all, N, home=False)
        out.update(compute_for_split_metric(last_away, last_away[METRIC].notnull(), f"Mark_{N}_Away"))

        # Zero rate + per-ToG rate (window)
        out[f"Mark_{N}_ZeroRate"] = to_python_scalar(zero_rate(recent[METRIC]))
        out[f"Marks_per100ToG_{N}"] = to_python_scalar(
            marks_per100_tog(recent[METRIC], recent.get(TOG_COL, pd.Series(dtype=float)))
        )
...
print(f"✅ Added/updated Mark features for {len(updates)} players in player_precomputes (season {season_year})")
