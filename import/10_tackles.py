# import/10_tackles_precomputes.py
...
# ---------- Tackles-only helpers ----------
METRIC = 'Tackles'              # column name in player_stats
...
def summary_stats(s: pd.Series) -> pd.Series:
    ...
    # light cap to tame outliers; tweak if you prefer
...
def tackles_per100_tog(series_vals: pd.Series, series_tog: pd.Series) -> float:
...
# Players to update (must exist in base precomputes)
...
# Optional timeslot splits:
# "Tackle_Season_Day", "Tackle_Season_Night", "Tackle_Season_Twilight"
...
for N in WINDOWS:
    recent_prefixes += [
        ...
        f"Tackle_{N}_Home",
        f"Tackle_{N}_Away",
        # optional Day/Night/Twilight
    ]

zero_cols = ["Tackle_Season_ZeroRate"] + [f"Tackle_{N}_ZeroRate" for N in WINDOWS]
rate_cols = ["Tackles_per100ToG_Season"] + [f"Tackles_per100ToG_{N}" for N in WINDOWS]
...
def compute_player_row(player: str) -> dict:
    ...
    if not p_season.empty:
        out.update(compute_for_split_metric(p_season, p_season[METRIC].notnull(), "Tackle_Season"))
        out.update(compute_for_split_metric(p_season, p_season['Conditions'].astype(str).str.lower() == 'dry', "Tackle_Season_Dry"))
        out.update(compute_for_split_metric(p_season, p_season['Conditions'].astype(str).str.lower() == 'wet', "Tackle_Season_Wet"))
        hamask_season = p_season.apply(is_home_game_row, axis=1)
        out.update(compute_for_split_metric(p_season, hamask_season, "Tackle_Season_Home"))
        out.update(compute_for_split_metric(p_season, ~hamask_season, "Tackle_Season_Away"))

        out["Tackle_Season_ZeroRate"] = to_python_scalar(zero_rate(p_season[METRIC]))
        out["Tackles_per100ToG_Season"] = to_python_scalar(
            tackles_per100_tog(p_season[METRIC], p_season.get(TOG_COL, pd.Series(dtype=float)))
        )
    else:
        for pfx in season_prefixes:
            for sfx in stat_suffixes:
                out[f"{pfx}_{sfx}"] = np.nan
        out["Tackle_Season_ZeroRate"] = np.nan
        out["Tackles_per100ToG_Season"] = np.nan

    # Recent windows (all years)
    for N in WINDOWS:
        recent = p_all.sort_values('Date').tail(N)
        out.update(compute_for_split_metric(recent, recent[METRIC].notnull(), f"Tackle_{N}"))

        last_dry = last_n_by(p_all, N, 'Conditions', 'dry')
        out.update(compute_for_split_metric(last_dry, last_dry[METRIC].notnull(), f"Tackle_{N}_Dry"))

        last_wet = last_n_by(p_all, N, 'Conditions', 'wet')
        out.update(compute_for_split_metric(last_wet, last_wet[METRIC].notnull(), f"Tackle_{N}_Wet"))

        last_home = last_n_home_away(p_all, N, home=True)
        out.update(compute_for_split_metric(last_home, last_home[METRIC].notnull(), f"Tackle_{N}_Home"))

        last_away = last_n_home_away(p_all, N, home=False)
        out.update(compute_for_split_metric(last_away, last_away[METRIC].notnull(), f"Tackle_{N}_Away"))

        out[f"Tackle_{N}_ZeroRate"] = to_python_scalar(zero_rate(recent[METRIC]))
        out[f"Tackles_per100ToG_{N}"] = to_python_scalar(
            tackles_per100_tog(recent[METRIC], recent.get(TOG_COL, pd.Series(dtype=float)))
        )
...
print(f"✅ Added/updated Tackle features for {len(updates)} players in player_precomputes (season {season_year})")
