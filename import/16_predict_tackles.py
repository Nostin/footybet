...
def resolve_models_dir() -> Path:
    candidates = [
        ROOT / "models" / "tackles",
        ROOT / "machine_learning" / "models" / "tackles",
    ]
    for p in candidates:
        if p.exists():
            return p.resolve()
    raise FileNotFoundError(
        "Could not find tackles model directory. Checked:\n" +
...
def enrich_tackles(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["wet_dry_tackles_ratio_last_3"] = safe_ratio(
        df["wet_tackles_last_3"],
        df["dry_tackles_last_3"]
    )
...
with engine.begin() as conn:
    for col in ["Prob_2_Tackles", "Prob_4_Tackles", "Prob_6_Tackles", "Prob_8_Tackles"]:
...
with engine.begin() as conn:
    for _, r in pred_out.iterrows():
        params = {
            "p2": to_python_scalar(r.get("Prob_2_Tackles")),
            "p4": to_python_scalar(r.get("Prob_4_Tackles")),
            "p6": to_python_scalar(r.get("Prob_6_Tackles")),
            "p8": to_python_scalar(r.get("Prob_8_Tackles")),
            "player": r["Player"],
        }

        sql = (
            'UPDATE player_precomputes '
            'SET "Prob_2_Tackles" = :p2, '
                '"Prob_4_Tackles" = :p4, '
                '"Prob_6_Tackles" = :p6, '
                '"Prob_8_Tackles" = :p8 '
            'WHERE "Player" = :player'
...
print(
    f"✅ Wrote tackles predictions for {len(pred_out)} players to player_precomputes "
