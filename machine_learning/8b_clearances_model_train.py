...
ODDS_COLS_BY_TARGET = {
    "c4": [
        "odds_clearances_4p", "odds_4plus_clearances", "odds_clearances_4plus"
    ],
    "c6": [
        "odds_clearances_6p", "odds_6plus_clearances", "odds_clearances_6plus"
    ],
    "c8": [
        "odds_clearances_8p", "odds_8plus_clearances", "odds_clearances_8plus"
    ],
}

df_train = pd.read_sql("SELECT * FROM model_feed_clearances_train", engine)
df_test  = pd.read_sql("SELECT * FROM model_feed_clearances_test", engine)
...
exact_wishlist = [
    "clearances_trend_last_5", "clearances_delta_5",
    "clearances_max_last_5", "clearances_min_last_5", "clearances_std_last_5",
    "is_home_game", "season_to_date_clearances_median",
    "form_minus_season_clearances_med_last_3",
    "team_clearances_avg_last_5", "opp_clearances_conc_last_5",
    "wet_clearances_last_3", "dry_clearances_last_3", "wet_dry_clearances_ratio_last_3",
    "missed_time_last4",
]

prefix_wishlist = [
    "clearances_cap_avg_last_", "clearances_cap_med_last_", "clearances_cap_max_last_",
    "clearances_cap_min_last_", "clearances_cap_var_last_",
    "clearances_cap_home_avg_last_", "clearances_cap_away_avg_last_",
    "clearances_cap_day_avg_last_", "clearances_cap_night_avg_last_", "clearances_cap_twilight_avg_last_",
    "clearances_floor_score_last_",
]
...
if not available_features_master:
    raise RuntimeError("No available features found for clearances. Check feature engineering output names.")
...
def eval_and_best_threshold(name, probs, y_true):
    ...
    print(
        f"📈 {name} best F1 threshold: {thr_f1:.3f} | "
...
for key, thresh in TARGETS:
    print("\n" + "=" * 70)
    print(f"🎯 Training ensemble for target: Clearances {thresh}+")
    ...
    def assert_all_finite(name, df_):
        bad = ~np.isfinite(df_.values)
        if bad.any():
            bad_cols = list(df_.columns[np.where(bad.any(axis=0))[0]])
    ...
    odds_col = next((c for c in ODDS_COLS_BY_TARGET[key] if c in te.columns), None)
    if odds_col is not None:
        te_out["odds_col_used"] = odds_col
        ...
        te_out["odds_col_used"] = np.nan

    os.makedirs("models/clearances", exist_ok=True)
    joblib.dump(lr_cal,   f"models/clearances/logreg_calibrated_{key}.pkl")
    joblib.dump(lgbm_cal, f"models/clearances/lgbm_calibrated_{key}.pkl")
    joblib.dump(scaler,   f"models/clearances/logreg_scaler_{key}.pkl")
    joblib.dump({"alpha": float(alpha)}, f"models/clearances/blend_alpha_{key}.pkl")

    all_out.append(te_out)

if all_out:
    out = pd.concat(all_out, ignore_index=True)
    out.to_sql("clearances_blend_preds_multi", engine, if_exists="replace", index=False)
    print("✅ Predictions saved to table: clearances_blend_preds_multi")
else:
