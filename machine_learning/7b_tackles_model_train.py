...
# Adjust these to match your actual schema if needed
ODDS_COLS_BY_TARGET = {
    "t2": [
        "odds_tackles_2p", "odds_2plus_tackles", "odds_tackles_2plus"
    ],
    "t4": [
        "odds_tackles_4p", "odds_4plus_tackles", "odds_tackles_4plus"
    ],
    "t6": [
        "odds_tackles_6p", "odds_6plus_tackles", "odds_tackles_6plus"
    ],
    "t8": [
        "odds_tackles_8p", "odds_8plus_tackles", "odds_tackles_8plus"
    ],
}

df_train = pd.read_sql("SELECT * FROM model_feed_tackles_train", engine)
df_test  = pd.read_sql("SELECT * FROM model_feed_tackles_test", engine)
...
exact_wishlist = [
    "tackles_trend_last_5", "tackles_delta_5",
    "tackles_max_last_5", "tackles_min_last_5", "tackles_std_last_5",
    "is_home_game", "season_to_date_tackles_median",
    "form_minus_season_tackles_med_last_3",
    "team_tackles_avg_last_5", "opp_tackles_conc_last_5",
    "wet_tackles_last_3", "dry_tackles_last_3", "wet_dry_tackles_ratio_last_3",
    "missed_time_last4",
]

prefix_wishlist = [
    "tackles_cap_avg_last_", "tackles_cap_med_last_", "tackles_cap_max_last_",
    "tackles_cap_min_last_", "tackles_cap_var_last_",
    "tackles_cap_home_avg_last_", "tackles_cap_away_avg_last_",
    "tackles_cap_day_avg_last_", "tackles_cap_night_avg_last_", "tackles_cap_twilight_avg_last_",
    "tackles_floor_score_last_",
]
...
if not available_features_master:
    raise RuntimeError("No available features found for tackles. Check feature engineering output names.")
...
for key, thresh in TARGETS:
    print("\n" + "=" * 70)
    print(f"🎯 Training ensemble for target: Tackles {thresh}+")
    ...
    os.makedirs("models/tackles", exist_ok=True)
    joblib.dump(lr_cal,   f"models/tackles/logreg_calibrated_{key}.pkl")
    joblib.dump(lgbm_cal, f"models/tackles/lgbm_calibrated_{key}.pkl")
    joblib.dump(scaler,   f"models/tackles/logreg_scaler_{key}.pkl")
    joblib.dump({"alpha": float(alpha)}, f"models/tackles/blend_alpha_{key}.pkl")

    all_out.append(te_out)

if all_out:
    out = pd.concat(all_out, ignore_index=True)
    out.to_sql("tackles_blend_preds_multi", engine, if_exists="replace", index=False)
    print("✅ Predictions saved to table: tackles_blend_preds_multi")
else:
