...
def enrich_marks(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def capped_marks_arr(x):
        # Marks can spike for certain roles; cap to stabilise variance
    ...
    df["wet_dry_marks_ratio_last_3"] = df["wet_marks_last_3"] / df["dry_marks_last_3"]
...
leakage_cols = [
    ...
    "Game Result", "Time on Ground %", "is_test", "Marks"
]
...
train_enriched.to_sql("model_feed_marks_train", con=engine, if_exists="replace", index=False)
print("✅ model_feed_marks_train saved")

test_enriched.to_sql("model_feed_marks_test", con=engine, if_exists="replace", index=False)
print("✅ model_feed_marks_test saved")